...
const getActivityIcon = (type: string) => {
  switch (type) {
    case "TASK_CREATED":
      return { icon: <Plus className="size-3.5 text-brand-primary" />, bg: "bg-brand-primary/10 border-brand-primary/30" };
    case "TASK_MOVED":
    case "STATUS_CHANGED":
      return { icon: <ArrowRight className="size-3.5 text-brand-secondary" />, bg: "bg-brand-secondary/10 border-brand-secondary/30" };
    case "ASSIGNEE_ADDED":
      return { icon: <UserPlus className="size-3.5 text-brand-primary" />, bg: "bg-brand-primary/10 border-brand-primary/30" };
    case "ASSIGNEE_REMOVED":
      return { icon: <X className="size-3.5 text-red-700" />, bg: "bg-red-50 border-red-200" };
    case "CHECKLIST_ITEM_COMPLETED":
      return { icon: <Check className="size-3.5 text-brand-primary" />, bg: "bg-brand-primary/10 border-brand-primary/30" };
    case "CHECKLIST_ITEM_ADDED":
    case "CHECKLIST_ITEM_INCOMPLETED":
      return { icon: <CheckSquare className="size-3.5 text-brand-secondary" />, bg: "bg-brand-secondary/10 border-brand-secondary/30" };
    case "COMMENT_ADDED":
      return { icon: <MessageSquare className="size-3.5 text-brand-primary" />, bg: "bg-brand-primary/10 border-brand-primary/30" };
    case "TASK_ARCHIVED":
      return { icon: <Archive className="size-3.5 text-muted" />, bg: "bg-background border-border" };
    case "TASK_RESTORED":
      return { icon: <RefreshCw className="size-3.5 text-brand-secondary" />, bg: "bg-brand-secondary/10 border-brand-secondary/30" };
    default:
      return { icon: <History className="size-3.5 text-muted" />, bg: "bg-background border-border" };
  }
};
...
const renderActivityDetails = (act: TaskActivityDto) => {
  ...
  switch (type) {
    ...
    case "STATUS_CHANGED":
      return (
        <span className="inline-flex items-center gap-1.5 flex-wrap">
          ...
          <ArrowRight className="inline size-3 text-muted" />
    ...
    case "COMMENT_ADDED":
      return (
        <span>
          commented: <span className="italic text-muted">&ldquo;{String(d.content || d.comment || "")}&rdquo;</span>
        </span>
      );
...
export function TaskDetailPanel({ groupId, taskId, onClose }: TaskDetailPanelProps) {
  ...
  if (isTaskLoading) {
    return (
      <TaskDrawerShell title="Loading task details" onClose={onClose}>
  ...
  if (taskError || !task) {
    return (
      <TaskDrawerShell title="Task detail error" onClose={onClose}>
  ...
  return (
    ...
    >
        <div className="min-w-0 space-y-6 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] min-[481px]:p-6">
          ...
          {!isEditingTitleDesc ? (
            ...
            >
              ...
              <div className="mt-2 break-words text-sm leading-relaxed whitespace-pre-wrap text-muted">
                {task.description || (
                  <span className="text-muted/50 italic">No description provided. Click to add.</span>
                )}
              </div>
            </div>
          ) : (
          ...
          {/* Metadata Grid */}
          <div className="grid grid-cols-2 gap-4 rounded-2xl border border-border bg-surface p-4 max-[480px]:grid-cols-1">
            <div>
          ...
          <div>
            ...
            {group?.members && group.members.length > 0 ? (
              ...
              <div className="text-xs text-muted/60 italic">Loading members list...</div>
            )}
          </div>
          ...
          <div>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              <h3 className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-muted m-0">
            ...
            <div className="space-y-2 mb-3">
              {task.checklistItems && task.checklistItems.length > 0 ? (
                ...
                <div className="py-4 text-center text-xs text-muted/50 border border-dashed border-border rounded-xl">
                  No checklist items.
                </div>
              )}
            </div>
                        ...
                        <div className="space-y-6">
                          {Object.keys(grouped).map((groupKey) => (
                            <div key={groupKey} className="space-y-3">
                              <h4 className="text-xs font-bold uppercase tracking-wider text-muted/80 pl-2 border-l-2 border-brand-secondary/40">
                                {groupKey}
                              </h4>
                                ...
                                {grouped[groupKey].map((act) => {
                                  ...
                                  return (
                                    <div key={act.id} className="relative min-w-0 text-xs">
                                      ...
                                      <div className="mb-0.5 flex min-w-0 flex-wrap items-center gap-1.5 text-muted/60">
